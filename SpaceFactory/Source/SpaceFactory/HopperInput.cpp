...
void AHopperInput::InputItem()
{
    ...
    if (!Machine->InputItem && PneumaticTube->TransportingItem)
    {
        UE_LOG(LogTemp, Warning, TEXT("%s recieved %s"), *Machine->GetName(),
               *PneumaticTube->TransportingItem->GetName());
        Machine->InputItem = PneumaticTube->TransportingItem;
...
bool AHopperInput::BuildSelf(ADetectorBuildTool *BuildTool)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        UE_LOG(LogTemp, Warning, TEXT("end vector is %s for HopperInput"),
               *(Start + (Rot.Vector() * DetectDistance)).ToString());
        GetWorld()->LineTraceSingleByChannel(Hit, Start, Start + (Rot.Vector() * DetectDistance),
        ...
        if (HitMachine && HitMachine->HopperInput.Num() >= 0)
        {
            UE_LOG(LogTemp, Warning, TEXT("assigned machine as: %s for HopperInput: %s"), *HitMachine->GetName(),
                   *GetName());
            Machine = HitMachine;
            HitMachine->HopperInput.Add(this);
            return true;
        }
    }
